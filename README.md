...
- **2026-06-21** — Relay pull-worker provider: external worker apps (a WebGPU browser tab, a phone running mlc-llm, a desktop running llama.cpp, anything that speaks HTTP+SSE) connect INBOUND to `pasclaw gateway`, advertise the models they can serve via `X-Relay-Capabilities`, pull pending inference requests off PasClaw's in-process queue, run them on their own hardware, and POST results back. Three new endpoints: `GET /v1/relay/poll` (SSE; long-polls for work), `POST /v1/relay/respond/<id>` (worker submits structured `tool_calls` + content + usage), `GET /v1/relay/status` (queue depth + per-worker registry). New catalog row `relay` + `pfRelay` family. Unlocks "use my laptop's GPU without exposing ports", "Replicate-hosted PasClaw served by inference on my phone", and "share a home GPU across devices". 14 hermetic tests (queue mechanics, Cancel under lock for timeout safety, broadcast wakeups, empty-on-either-side capability match for the onboarding case). ([#318])
- **2026-06-20** — `pasclaw plan` subcommand + `pasclaw build` PLAN.md auto-pickup + `pasclaw build --goal` Ralph wiring. `pasclaw plan -d "<task>"` runs the agent in `pmPlan` mode with a dedicated `plan_write` tool (the only `tcReadOnly` tool in the codebase that writes to disk; hard-coded target `workspace/PLAN.md`) and produces a structured markdown plan with `## Goal / Files / Steps / Open questions / Risks`. `pasclaw build` auto-picks-up `workspace/PLAN.md` as a `## Active Plan` system-prompt section (stale-check + `--no-plan` opt-out) and archives consumed plans to `workspace/memory/plans/<timestamp>.md` after success. `pasclaw build --goal` parses the plan's `## Goal` line and seeds `TGoalRunner` (the Ralph judge loop) with it. `cog-build/predict.py` gains a 4-choice `mode` Input (`build` / `plan` / `plan build` / `plan build goal`) with intermediate workspace zip chaining for the chained variants. Plus `ExtractGoalFromPlanFile` with an explicit `HomeOverride` parameter as a workaround for FPC's `libc_setenv` not propagating to `GetEnvironmentVariable` in the same process. ([#317])
- **2026-06-20** — Cloudflare AI Gateway provider catalog rows: `cloudflare` (compat OpenAI-shape endpoint at `gateway.ai.cloudflare.com/v1/<acct>/<gw>/compat`, default model `workers-ai/@cf/meta/llama-3.3-70b-instruct-fp8-fast` using the gateway's `<provider>/<model>` routing notation), `cloudflare-anthropic` (Anthropic passthrough with native `x-api-key` auth), `cloudflare-gemini` (Gemini passthrough with native `x-goog-api-key` auth). Pure catalog-only addition — existing `TOpenAIProvider` / `TAnthropicProvider` / `TGeminiProvider` units handle the wire shape without modification once `api_base` is set to the gateway URL. Six pinned tests in `provider_catalog_tests`. ([#316])
- **2026-06-20** — MCP progressive disclosure + Claude Code-style `tool_search` (Hermes Level 0/1 pattern, applied to MCP instead of skills). When `Cfg.MCPProgressiveDisclosure` is on (default True since fat catalogs like the Replicate MCP and GitHub MCP make eager-reveal too expensive), MCP tools register into the dispatcher as usual but with `IsDeferred=True` so `TToolRegistry.ToProviderDefs` strips their schemas from the per-request `tools` array. The system prompt gains a `## Deferred Tools` section listing names only, and a new `tool_search` tool loads schemas on demand via three query forms: `select:Name1,Name2` (exact fetch), `+required keyword1 keyword2` (require substring in name, rank rest), or plain keyword search across name + description. Revealed tools become callable on the very next iteration. Registry gains `RegisterDeferred(T, Deferred)` alongside plain `Register(T)` (defensively zeroes `IsDeferred` so legacy stack-built `TTool` records can't accidentally hide tools via uninitialised memory). ([#315])
- **2026-06-20** — Stock-config defaults adopt the bench-grounded lean-edit shape (`bench/swe/README.md`): `web_fetch` / `vault_tools` / `memory_fetch` OFF out-of-box (operators opt in via onboarding when they actually use them); the six zero-prompt-cost behavioral toggles ON by default (`orient_task_aware`, `checkpoints`, `stats`, 1h `prompt_cache.ttl`, `distiller`, `auto_router`). `HashlineEnabled` controls only `fs_edit_hashline` after split-gating; `fs_grep` registers unconditionally (its six ripgrep-inspired optimisations beat `shell_exec grep` on real codebases and Windows has no shell `grep`). Onboarding asks; `--no-hashline` CLI flag still works as per-run override. ([#314])
- **2026-06-18** — Web UI gains a workspace.zip export + import surface from the gateway: download a snapshot of `$PASCLAW_HOME` (memory, sessions, kb, checkpoints, skills, scars) as a single zip for hand-off between machines, and re-import to restore. Same shape as the `pasclaw build` workspace handshake but operator-driven through the browser. ([#312])
- **2026-06-18** — Model-callable `cron` tool (opt-in via `cron_tool_enabled: true` in `config.json`; off by default since it grants background autonomy). Lets the agent schedule an existing **skill** on a cron expression, e.g. "every weekday at 9am, run the daily-summary skill". Schedules persist in `workspace/cron.json`; the scheduler picks them up on next `pasclaw serve` / `pasclaw gateway` start. ([#310])
- **2026-06-18** — `memory_search` / `kb_search` FTS5 snippet window widened from 24 → 60 characters and the system prompt now nudges the model to follow up with a targeted `fs_read` when a snippet looks relevant but lacks context. Cuts the "snippet was so tight I can't tell what it's about" failure mode without ballooning per-result token cost. ([#309])
- **2026-06-18** — Windows `cmd.exe` quoting fix in `shell_exec` / `execute_code`. Embedded double-quote literals in arguments (`hello "world"`) now survive the trip through `cmd.exe /c` correctly; previously the inner quotes were eaten and the command saw `hello world`. ([#307])
- **2026-06-17** — `cog-build` honours `profile=security`'s sandbox tightening (workspace restriction + shell deny + private-network block) by intentionally **not** seeding any sandbox fields in the cog's generated `config.json` — operators choosing the security profile actually get its hardening, not the cog's defaults silently shadowing it. Same PR drops a misleading `web_search` claim in the cog README (the tool only registers when a search provider is configured; the cog doesn't supply one — use `web_fetch`). ([#306])
- **2026-06-17** — `cog-build` defaults `profile` to **`max-build`** — the richest unattended-build toolset (`web_fetch`, `vector_search`, `auto_router`, 1h `prompt_cache`, `condense_reversible`, `orient_task_aware`, `promptware`, the self-improving-skills suite, vault tools, `tool_output_cap` 16384). Operators picking different posture override to `baseline` / `low-token` / `security` / `all-on` / empty. Profile layered UNDER the cog's explicit `config.json` fields so the cog's provider catalog + `checkpoints_enabled` always win. ([#304])
- **2026-06-17** — `cog-build` outputs as `list[Path]` (two CDN URLs: `workspace_out_xxxx.zip` + `reply_out_xxxx.txt`) instead of a nested-Path `BaseModel` (which Cog's older runtimes base64-encoded inline instead of uploading). API keys move to `Optional[Secret]` Cog inputs — stored encrypted on Replicate, masked in the UI, never appear in prediction-input logs. `-q` flag forwarded to `pasclaw build` so the ASCII banner stays out of `reply.txt`. ([#303])
- **2026-06-16** — `cog-build` makes `workspace_in` optional (Cog's `Optional[Path]` plus a `workspace_in_url` string for explicit-URL `pget` parallelism). Leave both empty for a fresh run; the cog auto-creates a tempdir `$PASCLAW_HOME`. ([#302])
- **2026-06-16** — `pasclaw build` command + `cog-build/` Replicate cog predictor with **workspace.zip handshake**. `pasclaw build -d "<task>"` is a thin orchestrator over `pasclaw agent --quiet`: optional `--workspace-in <zip>` unpacks into `$PASCLAW_HOME` before the agent runs, `--workspace-out <zip>` packs `$PASCLAW_HOME` afterwards. The cog wraps this for Replicate so callers can run long-horizon builds in 30-second invocations and round-trip the entire agent brain (memory, kb, sessions, checkpoints, project files written) through one zip URL per call. Default 50 iterations; 4 GiB workspace cap; subprocess timeout 3600s. Sibling to the existing `cog/` (which runs the simpler one-shot `pasclaw agent`). ([#300])
- **2026-06-16** — Checkpoints backend switches to **zpaq** for the snapshot store, enabling `/redo` on top of the existing `/undo`. The zpaq archive at `workspace/checkpoints/<session>/archive.zpaq` gives content-addressable storage with dedupe-by-default, so multi-edit sessions don't bloat the snapshot history. `/undo` reverts the most recent `fs_write` / `fs_edit_hashline`; `/redo` re-applies. End-to-end through the TUI + interactive CLI. ([#299])
- **2026-06-16** — `AGENTS.md` project-rules convention (opencode / Codex / Cursor / Zed shape) read from cwd walking up to git root + injected as the system prompt's `## Project Rules` section. New `pasclaw init` command scans cwd and asks the model for a starter `AGENTS.md` (one-shot, no tool loop); `/init` slash command does the same inside an interactive session. ([#298])
- **2026-06-16** — `tool_choice` supports force-a-specific-tool form across providers. Pass a tool / function NAME (anything that isn't `auto` / `none` / `required`) and each provider emits its native object shape: OpenAI `type=function, function.name=<X>`, Anthropic `type=tool, name=<X>`. Gemini ignores `tool_choice` entirely. Useful for "must call this specific tool on this turn" workflows without prompt-shaping. ([#297])
- **2026-06-16** — Delphi build `IShellBackend` env-injection fix: `ExtraEnv` (used by the test harness to point at sandboxed cron paths, by `pasclaw build` to inject `PASCLAW_HOME`, etc.) now reaches spawned children on Delphi/Windows AND on the Delphi/POSIX paths. Was silently dropped before; FPC paths were already correct. ([#296])
- **2026-06-16** — Knowledgebase native PDF ingest: parses PDF text via FlateDecode + `/ToUnicode` lookup in pure Pascal (no Poppler / `pdftotext` dependency). `pasclaw kb add <file>.pdf` works without preprocessing. Best-effort — scanned PDFs (image-only, no embedded text) still need an external OCR step. ([#295])
- **2026-06-15** — `pasclaw profile diff <a> <b>` (side-by-side comparison of two profiles' resolved field values) + `pasclaw profile bench <profile>` (run the SWE bench fixtures under the named profile and emit aggregated turn-count / tool-call / token stats). Finishes the profile-system follow-ups: profile authors now have a workflow to A/B their changes against the bench and pick the right floor. ([#292])
...
- **2026-06-12** — `dcc64 F2613 Unit 'PasClaw.Otel' not found` fixed by appending `..\pkg\otel` to the Delphi project's `DCC_UnitSearchPath`. Same PR also adds `pasclaw agent --quiet` / `-q` for machine-readable single-turn output: drops the banner, the `assistant (provider/model):` header, every per-tool decoration line, and the trailing `[tokens in=... out=...]` summary. Exit code is non-zero on provider misconfiguration or a failed tool loop, so scripts can detect failures without parsing stdout. ([#243])
- **2026-06-12** — OpenTelemetry traces (OTLP/HTTP+JSON) on the agent loop + gateway HTTP server, off by default. Span hierarchy: `HTTP <method> <route>` (gateway, W3C `traceparent` parsed for parent context) → `openclaw.agent.turn` → `chat <model>` (provider request with `gen_ai.*` attrs) → `execute_tool <name>` (serial tool dispatch). Span shape mirrors openclaw v2026.2+ so Langfuse / Tempo / Jaeger / Honeycomb / Datadog dashboards work without remapping. Flip on via `diagnostics.otel.enabled: true` in `config.json` OR the standard `OTEL_EXPORTER_OTLP_ENDPOINT` env var. ([#242])
- **2026-06-12** — `fs_grep` ripgrep tier 5+6: byte walker replacing `TStringList` / `StringReplace` / per-line `LowerCase` (tier 5), and Boyer-Moore-Horspool substring search replacing `Pos()` (tier 6). 3-5x faster on the per-file path for typical source files; ~3-10x fewer text-byte reads on patterns the model actually queries (`TODO`, `function`, `class`, `FIXME`). Output byte-identical to the old path. ([#241])
- **2026-06-12** — `fs_grep` ripgrep-inspired tier 1-4 speedups: defer `ComputeFileHash` until the first match in a file (tier 1); skip well-known VCS / build / deps directories by name at walk time (`.git`, `.hg`, `.svn`, `node_modules`, `target`, `build`, `dist`, `vendor`, `.venv`, `__pycache__`, `.gradle`, `.next`) — tier 2; NUL-byte binary sniff in first 1 KiB (tier 3); file-size cap (default 10 MiB, override via `max_file_bytes` arg) — tier 4. Together these pull the typical scan envelope from "read every file under the root" to "read the source files only". ([#240])
- **2026-06-11** — Windows `shell_exec` output: auto-detect UTF-8 vs OEM codepage with strict `MB_ERR_INVALID_CHARS` UTF-8 first, fall back to `GetOEMCP`. Catches `pwsh` (PowerShell 6+ defaults to UTF-8 piped output) AND `cmd.exe` (defaults to OEM) without requiring operator config. Replaces an earlier `GetConsoleOutputCP` approach which broke real-world non-ASCII paths like `r�sum�.txt`. ([#239])
...
- **2026-06-11** — Windows `shell_exec` output: decode child-process stdout via the OEM codepage instead of assuming UTF-8. Catches `dir /b`, `git status`, `git diff` mojibake on non-ASCII paths. Subsequently refined in #239 to also handle `pwsh`'s UTF-8 piped output. ([#237])
- **2026-06-10** — Shell backends (`local` | `docker`) via `IShellBackend`. `shell_exec` and `execute_code` dispatch through an active backend instead of going straight to `/bin/sh -c`. The `docker` backend spawns a per-session container at session start, `docker exec`s into it for every shell call, bind-mounts the workspace at the **same path** inside the container (so `fs_*` from the host and `cat`/`tee` from inside reference the same files — no path translation), and stops it at session end. Hardening win: even a denylist-escaping model can only touch the container, not the host filesystem. `~/.pasclaw/config.json` (provider keys) is deliberately **not** mounted in. Phase 1 wires this for `pasclaw agent` (interactive + one-shot) and `pasclaw heartbeat`. ([#233])
- **2026-06-10** — Heartbeat daemon (off by default; `pasclaw onboard` asks to enable). `pasclaw heartbeat` reads `workspace/heartbeat.md` every N minutes (default 30), runs the full agent loop on the body, and optionally posts the reply to a named channel. Each tick re-reads the content file so edits land on the next tick without restarting the daemon. Use cases: "every hour, check whether the staging build is red over an hour and message me if so"; "every 6h, summarise GitHub issue mentions and post to the team Slack". Same PR also wires **reversible condensation (CCR)** — destructive condensers (JSON shrink, shell-output filters) stash the original under a fresh `tool_output_get` handle and append a footer naming it, so the model defaults to the structural view but can recall the verbatim bytes via one tool call. ([#232])
- **2026-06-10** — Three feature batch: (1) `send_message` model-facing tool — post a message to a named, operator-configured channel mid-task (`channels[]` in `config.json`); the model can only reach endpoints the operator pre-declared (no model-supplied URLs). (2) ~14 more per-command shell-output condenser families: compile walls (`cargo build`/`check`/`clippy`), linters (eslint stylish), package managers (`npm install`/`pip install`), build systems (`mvn`/`gradle`), IaC (`terraform plan`/`apply`), tabular CLIs (`docker ps`, `kubectl get`, `gh`), JSON output (`aws`). (3) Promptware defense — substring-pattern scan over three indirect-input chokepoints (tool output, recalled memory, stored skills) with annotate-don't-block semantics for chokepoints 1-2 and suppress-outright for SKILL.md descriptions; rule families cover instruction overrides, fake system prompts, concealment, system-prompt exfiltration probes, curl-pipe-to-shell. (4) Task-aware MEMORY.md slicing via `orient_task_aware: true` — slice each memory file into markdown sections, score against the current task by lexical overlap, inject only the sections that score > 0. ([#230])
- **2026-06-09** — `pasclaw /goal` Ralph loop with judge model: auto-continues turn after turn until a judge model verdict of `MET` / `FAILED` / `CONTINUE` is reached. Atlas-style "ralph wiggum" pattern — keep nudging the agent until the goal is provably done. Same PR also adds a JSON-aware tool-output condenser that collapses MCP search responses into structural summaries while preserving keys (so the model can still reason about the shape). ([#223])
...
- **2026-06-09** — Inbound MCP server mode: `pasclaw gateway` (and the new `pasclaw mcp stdio` subcommand) serve PasClaw's curated tools (`memory_search`, `kb_search`, `SCARS`, etc.) over MCP so other agents (Claude Code, Cursor, etc.) reach PasClaw's state live. The inbound MCP server defaults to read-only; `--mcp-allow-write` opts in to mutating tools. `--mcp-port <p>` spawns a second listener on the named port that honours only MCP routes plus `/v1/health` (isolation for different firewall rules). ([#218])
- **2026-06-09** — `pasclaw onboard` offers to index documents into the knowledgebase as part of setup. ([#217])
- **2026-06-09** — Knowledgebase: RAG over operator-curated documents (`pasclaw kb add ~/docs/`). FTS5 BM25 ranking by default; auto-upgrades to **hybrid keyword + vector** with Reciprocal Rank Fusion when the local embedding runtime is provisioned (`pasclaw memory provision` — same sqlite-vec + ONNX Runtime + MiniLM artifacts `memory_search` uses). Two read-only tools (`kb_search` / `kb_get`) auto-register once at least one source is registered. Documents are indexed **in place** (never copied) into `workspace/kb.db`. Supports markdown, plain text, HTML, and common source-code extensions; PDFs require external conversion. Unlike `memory_search`, the KB does NOT re-index on every query — `pasclaw kb sync` is the explicit refresh, keeping `kb_search` fast on large corpora. ([#214])
...
**Shell backends (local | docker)** — `shell_exec` and `execute_code` dispatch through an `IShellBackend` interface so the operator can pick where commands actually run. **Local** (default) is `/bin/sh -c` in the host process — the legacy behaviour. **Docker** (`"shell_backend": "docker"` in `config.json` or `--backend docker` per-run) spawns a per-session container at session start, `docker exec`s into it for every shell call, and stops it at session end. The workspace is bind-mounted at the **same path** inside the container, so `fs_read`/`fs_write` from the host process and `cat`/`tee` from inside the container reference the same files — operators inspect from outside with `ls ~/.pasclaw/workspace/`, no path translation. Hardening win: even a model that escapes the `ShellAllowed` denylist can only touch the container, not the host filesystem; `~/.pasclaw/config.json` (provider keys) is deliberately **not** mounted in. Image / network / privileged / user are configurable; the default `debian:bookworm-slim` is small and universally available. Onboarding asks once; CLI override `--backend docker|local` doesn't persist. Phase 1 wires this for `pasclaw agent` (interactive + one-shot) and `pasclaw heartbeat`; `pasclaw tui` / `pasclaw serve` / `pasclaw gateway` refuse docker cleanly until per-session container handoff inside those long-running surfaces lands. SSH backend is Phase 2. Inspired by openclaw's six terminal backends and hermes-agent's execution-environment plumbing.
