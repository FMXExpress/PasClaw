...
- **2026-06-09** — Inbound MCP server mode: `pasclaw gateway` (and the new `pasclaw mcp stdio` subcommand) serve PasClaw's curated tools (`memory_search`, `kb_search`, `SCARS`, etc.) over MCP so other agents (Claude Code, Cursor, etc.) reach PasClaw's state live. `--mcp-allow <list>` allowlist, `--mcp-read-only` refuses mutating tools, `--mcp-only --mcp-port <p>` spins a second listener that honours only MCP routes. ([#218])
