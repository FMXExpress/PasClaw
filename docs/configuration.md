...
| `fast_model` | *(resolved)* | Model for work that doesn't need the good one — currently search/data/report answer pages, on the **primary** provider. Empty resolves one: the auto-router's easy tier when that router is on (`easy_provider` **and** `easy_model` together, since the model is an override on that provider), else a known small model for the provider in use (Gemini → Flash Lite, Anthropic → Haiku, OpenAI → mini), else the main model unchanged. **Deep research never uses it** — that one plans, reads several sources and synthesises, which is what the main model is for. |
...
| `compaction.recent_turns` | `8` | Floor: never keep fewer than this many recent messages. |
| `compaction.summary_budget` | `800` | Token budget the summariser is told to stay under. |
