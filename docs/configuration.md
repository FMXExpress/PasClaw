...
| `fast_model` | *(resolved)* | Model for work that doesn't need the good one — currently search/data/report answer pages. Empty resolves one: `auto_router.easy_model` when that router is on, else a known small model for the provider in use (Gemini → Flash Lite, Anthropic → Haiku, OpenAI → mini), else the main model unchanged. **Deep research never uses it** — that one plans, reads several sources and synthesises, which is what the main model is for. |
