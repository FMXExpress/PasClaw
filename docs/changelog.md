...
- **2026-06-20** — `pasclaw plan` subcommand + `pasclaw build` PLAN.md auto-pickup + `pasclaw build --goal` Ralph wiring. `pasclaw plan -d "<task>"` runs the agent in `pmPlan` mode with a dedicated `plan_write` tool (the only `tcReadOnly` tool in the codebase that writes to disk; hard-coded target `workspace/PLAN.md`) and produces a structured markdown plan with `## Goal / Files / Steps / Open questions / Risks`. `pasclaw build` auto-picks-up `workspace/PLAN.md` as a `## Active Plan` system-prompt section (stale-check + `--no-plan` opt-out) and archives consumed plans to `workspace/memory/plans/<timestamp>.md` after success. `pasclaw build --goal` parses the plan's `## Goal` line and seeds `TGoalRunner` (the Ralph judge loop) with it. `cog-build/predict.py` gains a 4-choice `mode` Input (`build` / `plan` / `plan build` / `plan build goal`) with intermediate workspace zip chaining for the chained variants. ([#317])
- **2026-06-20** — Cloudflare AI Gateway provider catalog rows: `cloudflare` (compat OpenAI-shape endpoint, default model `workers-ai/@cf/meta/llama-3.3-70b-instruct-fp8-fast` using the gateway's `<provider>/<model>` routing notation), `cloudflare-anthropic` (Anthropic passthrough with native `x-api-key` auth), `cloudflare-gemini` (Gemini passthrough with native `x-goog-api-key` auth). Pure catalog-only addition — existing `TOpenAIProvider` / `TAnthropicProvider` / `TGeminiProvider` units handle the wire shape without modification once `api_base` is set to the gateway URL. ([#316])
- **2026-06-20** — MCP progressive disclosure + Claude Code-style `tool_search` (Hermes Level 0/1 pattern, applied to MCP). When `Cfg.MCPProgressiveDisclosure` is on (default True since fat catalogs like the Replicate MCP and GitHub MCP make eager-reveal too expensive), MCP tools register into the dispatcher with `IsDeferred=True` so `TToolRegistry.ToProviderDefs` strips their schemas from the per-request `tools` array. The system prompt gains a `## Deferred Tools` section listing names only, and `tool_search` loads schemas on demand via `select:Name1,Name2`, `+required keyword1 keyword2`, or plain keyword search. Revealed tools are callable on the next iteration. Registry gains `RegisterDeferred(T, Deferred)` alongside plain `Register(T)` (which defensively zeroes `IsDeferred` so legacy stack-built `TTool` records can't accidentally hide tools via uninitialised memory). ([#315])
- **2026-06-20** — Stock-config defaults adopt the bench-grounded lean-edit shape: `web_fetch` / `vault_tools` / `memory_fetch` OFF out-of-box; the six zero-prompt-cost behavioral toggles (`orient_task_aware`, `checkpoints`, `stats`, 1h `prompt_cache.ttl`, `distiller`, `auto_router`) ON by default. `HashlineEnabled` controls only `fs_edit_hashline` after split-gating; `fs_grep` registers unconditionally (its six ripgrep-inspired optimisations beat `shell_exec grep` on real codebases and Windows has no shell `grep`). ([#314])
...
- **2026-06-18** — Model-callable `cron` tool (opt-in via `cron_tool_enabled: true`; off by default since it grants background autonomy). Lets the agent schedule an existing skill on a cron expression. Schedules persist in `workspace/cron.json`; the scheduler picks them up on next `pasclaw serve` / `pasclaw gateway` start. ([#310])
- **2026-06-18** — `memory_search` / `kb_search` FTS5 snippet window widened from 24 → 60 characters and the system prompt now nudges the model to follow up with a targeted `fs_read` when a snippet looks relevant but lacks context. ([#309])
- **2026-06-18** — Windows `cmd.exe` quoting fix in `shell_exec` / `execute_code`. Embedded double-quote literals in arguments now survive the trip through `cmd.exe /c`; previously the inner quotes were eaten. ([#307])
...
- **2026-06-17** — `cog-build` outputs as `list[Path]` (two CDN URLs: `workspace_out_xxxx.zip` + `reply_out_xxxx.txt`) instead of a nested-Path `BaseModel`. API keys move to `Optional[Secret]` Cog inputs — stored encrypted on Replicate, masked in the UI. `-q` flag forwarded to `pasclaw build` so the ASCII banner stays out of `reply.txt`. ([#303])
- **2026-06-16** — `cog-build` makes `workspace_in` optional (Cog's `Optional[Path]` plus a `workspace_in_url` string for explicit-URL `pget` parallelism). Leave both empty for a fresh run. ([#302])
- **2026-06-16** — `pasclaw build` command + `cog-build/` Replicate cog predictor with workspace.zip handshake. `pasclaw build -d "<task>"` is a thin orchestrator over `pasclaw agent --quiet`: `--workspace-in <zip>` unpacks into `$PASCLAW_HOME` before the agent runs, `--workspace-out <zip>` packs it afterwards. The cog wraps this for Replicate so callers can run long-horizon builds in 30-second invocations and round-trip the entire agent brain through one zip URL per call. Default 50 iterations; 4 GiB workspace cap. Sibling to the existing `cog/`. ([#300])
- **2026-06-16** — Checkpoints backend switches to **zpaq** for the snapshot store, enabling `/redo` on top of the existing `/undo`. Content-addressable storage with dedupe-by-default at `workspace/checkpoints/<session>/archive.zpaq`. ([#299])
...
- **2026-06-16** — `tool_choice` supports force-a-specific-tool form across providers. Pass a tool / function NAME and each provider emits its native object shape (OpenAI `type=function, function.name=<X>`, Anthropic `type=tool, name=<X>`). Gemini ignores `tool_choice` entirely. ([#297])
- **2026-06-16** — Delphi build `IShellBackend` env-injection fix: `ExtraEnv` now reaches spawned children on Delphi/Windows AND Delphi/POSIX paths. Was silently dropped before; FPC paths were already correct. ([#296])
- **2026-06-16** — Knowledgebase native PDF ingest: parses PDF text via FlateDecode + `/ToUnicode` lookup in pure Pascal (no Poppler / `pdftotext` dependency). `pasclaw kb add <file>.pdf` works without preprocessing. ([#295])
- **2026-06-15** — `pasclaw profile diff <a> <b>` (side-by-side comparison of two profiles' resolved field values) + `pasclaw profile bench <profile>` (run the SWE bench fixtures under the named profile and emit aggregated turn-count / tool-call / token stats). Finishes the profile-system follow-ups. ([#292])
...
- **2026-06-09** — Inbound MCP server mode: `pasclaw gateway` (and `pasclaw mcp stdio`) serve PasClaw's curated tools (`memory_search`, `kb_search`, `SCARS`, ...) over MCP so other agents (Claude Code, Cursor, etc.) reach PasClaw's state live. The inbound MCP server defaults to read-only; `--mcp-allow-write` opts in to mutating tools. `--mcp-port <p>` spawns a second listener on the named port that honours only MCP routes plus `/v1/health` (isolation for different firewall rules). ([#218])
