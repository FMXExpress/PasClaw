...
| [`runbook`](#runbook) | Ask the agent to write a starter `AGENTS.md`. |
...
One-shot agent invocation: probe the current project and write a starter AGENTS.md. Pairs with `execute_code` so the probe is one tool call.
