...
## 2b. The generated web: search results as pages, and dreamed sites

Two influences, one mechanism. [Websim](https://websim.com/) and Gem Browser
generate the *web itself* — type a URL or a wish and HTML comes back. And the
Perplexity-style search PasClaw already does (`web_search` has 6 providers,
including Perplexity citation collation and Gemini grounding) currently ends as
chat text. On a desktop, it should end as a **page**.
...
- **Search-as-a-page.** The Browser window grows a search box (period-correct:
  ...
  sections, tables, images where warranted, and **citations as real links**.
  Endpoint: `POST /v1/pages {query, mode: "search"}` → job → page saved under a
  per-workspace `pages/` project (the browsing history *is* a project) → SSE
  `page-ready` → the Browser window navigates to it. Follow-up queries from that
  page's search box thread into the same session, so pages can refine pages.
- **Dreamed sites (websim mode).** Type a URL that doesn't exist — or exists but
  you'd rather imagine it — and the agent invents the page: `mode: "dream"`, no
  search grounding, pure synthesis. Links inside a dreamed page point back at
  `/v1/pages?mode=dream&url=...`, so **clicking a link dreams the next page** —
  an infinite generated web, one click deep at a time, cached in `pages/` so the
  same URL revisits the same dream. This is also the §0 vision at its purest:
  browsing 1996 GeoCities that never existed, in a Win95 window, generated by an
  intelligence the era could only put in a README about the future.
- **Provenance is non-negotiable.** The Gizmodo piece's core worry is generated
  text masquerading as the real web. So every page carries a status-bar strip
  the templates cannot omit: `GROUNDED — 6 sources` (each a link) for search
  pages, `DREAMED — this page is fiction` for websim pages. The two modes get
  visibly different chrome; a dreamed page is never allowed to look like a
  grounded one.
- **Promotion path.** A page is a proto-app. "Make this interactive" on any page
  converts it: the agent copies it into a new project as `kind: html`, adds JS
  (and the state store from §2 if needed), and the desktop swaps the document
  window for an app window. Search result → dashboard, dreamed store → working
  tracker. This one gesture is the product's whole pitch in miniature.
...
| **4b. Generated web** | `POST /v1/pages` (search + dream modes), `pages/` history project, Browser-window search box + dreamed-link navigation, provenance chrome, page→app promotion | gateway, both clients, promptware |
...
- **Dreamed pages are hallucination as a feature — keep the frame honest.** The
  provenance chrome (§2b) is mandatory, dreamed pages never fetch or exfiltrate
  (no grounding, no forms that submit anywhere), and a dream-per-click costs a
  model call — so dreamed navigation gets a visible per-session budget/counter,
  in period style ("You have 42 credits remaining"), rather than a silent bill.
