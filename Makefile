...
.PHONY: all clean run test openssl-1.0 test-doctor test-provider-retry smoke lint-pascal-shape lint-version lint-utf8-file-io test-workspaces test-projects test-tool-schema test-teams test-apps test-mail test-desktop-routes test-desktop test-hashline test-toolview test-anthropic-server-tools test-openai-server-tools test-tool-choice test-responses-tool-choice test-println-helper test-utf8-codepage-tag test-json-utf8-roundtrip test-stats-attribution test-sqlite-hint test-memory-search-roundtrip test-shell-cwd-report lint-studio test-read-file-encoding test-db-tools test-session-port test-json-lenient test-rerank test-sentencepiece test-rerank-eval test-model-discovery test-cron-tool test-provider-catalog test-output-cache test-agents test-working-state test-compact test-prune test-ansi-width test-shell-filters test-learn test-stream-reliability test-kb-index test-kb-pdf test-agents-md test-checkpoints-zpaq test-orient-preamble test-component-config test-autoroute-apply test-fallback-models test-memory-distill test-memory-facts test-memory-static-embed test-memory-autodistill test-checkpoints-redo test-build-roundtrip test-delphi-build print-version get-indy webui-res browser
...
test-skills-provenance: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/skills_provenance_tests.pas -o$(BUILDDIR)/skills_provenance_tests
	@$(BUILDDIR)/skills_provenance_tests

test-ssrf-guard: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/ssrf_guard_tests.pas -o$(BUILDDIR)/ssrf_guard_tests
	@$(BUILDDIR)/ssrf_guard_tests
test-shell-denylist-bypass: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
...
test-relay-servertools: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/relay_servertools_tests.pas -o$(BUILDDIR)/relay_servertools_tests
	@$(BUILDDIR)/relay_servertools_tests

# /v1/stats by_model must credit the model that actually spent the tokens.
# Buckets keyed by endpoint alone attributed a whole cumulative total to
# whichever model ran last (150,015 tokens credited to one that used 15).
test-stats-attribution: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/stats_attribution_repro.pas -o$(BUILDDIR)/stats_attribution_repro
	@PASCLAW_HOME=$$(mktemp -d) ; export PASCLAW_HOME ; \
	$(BUILDDIR)/stats_attribution_repro ; rc=$$? ; \
...
test-teams: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	@rm -rf $(BUILDDIR)/teams-test-home
	$(FPC) $(FPCFLAGS) src/tests/teams_tests.pas -o$(BUILDDIR)/teams_tests
	@PASCLAW_HOME=$(BUILDDIR)/teams-test-home $(BUILDDIR)/teams_tests
...
# that DO compile.
# TStrings.SaveToFile / LoadFromFile without an encoding write and read the
# system ANSI codepage on Delphi, mangling every non-ASCII character. FPC
# passes bytes through, so no compiled test here can catch it -- this pins
# the source shape instead. Vendor trees excluded except the localvector
# tokenizer loader, which PasClaw patches.
lint-utf8-file-io:
	@python3 scripts/check-utf8-file-io.py \
	  $$(find src/pkg src/cmd -name '*.pas' -not -path 'src/pkg/vendor/*') \
	  src/pkg/memory/localvector/LocalVector.SentencePiece.pas

# The product version lives in four places a release has to touch, and the
# Delphi build ships VersionFallback verbatim (no PASCLAW_VERSION env-var
# equivalent), so a stale value goes out on every Windows binary. Fails when
# the source is BEHIND the newest tag, or when the four disagree; skips the
# tag half cleanly when no tags are reachable.
lint-version:
...
test-desktop: lint-pascal-shape test-workspaces test-projects test-tool-schema test-teams test-apps test-mail test-workspace-isolation test-desktop-routes test-client-markdown test-client-api
...
test-agents: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	@rm -rf $(BUILDDIR)/agents-test-home
	$(FPC) $(FPCFLAGS) src/tests/agents_tests.pas -o$(BUILDDIR)/agents_tests
	@PASCLAW_HOME=$(BUILDDIR)/agents-test-home $(BUILDDIR)/agents_tests
...
# Conversation-history compaction: the rolling summary block, the
# tool-boundary cut, the retention budget + floor, the overflow
# classifier, and the config defaults staying equal to the code's.
test-compact: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/compact_tests.pas -o$(BUILDDIR)/compact_tests
	@$(BUILDDIR)/compact_tests

# LLM-guided pruning: the guarantees the pruner model is NOT trusted to
# honour -- omitted-means-kept, atomic tool groups, protected system and
# recent turns, verbatim survivors.
test-prune: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
...
# `pasclaw doctor` -- both output modes plus the exit-code contract.
# Drives the real binary against synthetic PASCLAW_HOMEs, so it needs $(BIN).
test-doctor: $(BIN) | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/doctor_tests.pas -o$(BUILDDIR)/doctor_tests
	@$(BUILDDIR)/doctor_tests

test-provider-retry: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/provider_retry_tests.pas -o$(BUILDDIR)/provider_retry_tests
	@$(BUILDDIR)/provider_retry_tests
...
# The prompt's parallel-batching rule vs the registry categories it asserts:
# the rule names tools as parallel-safe, but PasClaw.Tools.FS owns whether
# they actually are, so the two can drift into a silent lie.
# The per-turn clock: it must reach the model AND stay out of the cached
# prefix (the system block carries the cache_control breakpoint).
test-turn-clock: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/turn_clock_tests.pas -o$(BUILDDIR)/turn_clock_tests
	@$(BUILDDIR)/turn_clock_tests

test-prompt-batching: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/prompt_batching_tests.pas -o$(BUILDDIR)/prompt_batching_tests
	@$(BUILDDIR)/prompt_batching_tests
...
# Interfaces must not lean on FPC-only RTL types (dcc64 is not in CI).
test-delphi-iface-types: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/delphi_iface_types_tests.pas -o$(BUILDDIR)/delphi_iface_types_tests
	@$(BUILDDIR)/delphi_iface_types_tests

# Cooperative cancellation: the operator's pause actually stopping a turn.
test-tool-cancel: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/tool_cancel_tests.pas -o$(BUILDDIR)/tool_cancel_tests
	@$(BUILDDIR)/tool_cancel_tests
...
# Per-call `model` override on spawn: arg > spec.Model > parent DefaultModel.
test-subagent-model-arg: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
...
# MCP tools/call result parser: structuredContent + non-text blocks survive
# as ResultJSON so a workflow can chain one tool's output into the next.
test-mcp-result: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/mcp_result_tests.pas -o$(BUILDDIR)/mcp_result_tests
	@$(BUILDDIR)/mcp_result_tests

# Tabular re-encoding of MCP tool results: pins the conversions, the escaping
# and -- more importantly -- the refusals, since a half-reshaped result is
# worse than an unconverted one.
# Round-trip of the MCP behaviour flags. Both are written only when turned
# OFF, so a missing key in ToJSON silently re-enables the behaviour on the
# next SaveConfig -- a load-side test cannot see it.
test-config-mcp-flags: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/config_mcp_flags_tests.pas -o$(BUILDDIR)/config_mcp_flags_tests
	@$(BUILDDIR)/config_mcp_flags_tests

test-mcp-compact: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/mcp_compact_tests.pas -o$(BUILDDIR)/mcp_compact_tests
	@$(BUILDDIR)/mcp_compact_tests

# Workflow engine: schema parse/validate + topo execution with an injectable
# tool caller (no live MCP needed) + dotted/[i] selector evaluation.
test-workflow: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/workflow_tests.pas -o$(BUILDDIR)/workflow_tests
	@rm -rf $(BUILDDIR)/workflow-test-home
	@PASCLAW_HOME=$(BUILDDIR)/workflow-test-home $(BUILDDIR)/workflow_tests
...
# The dependency-free embedding tier, plus the embedder-identity contract
# that keeps two embedding spaces from being compared as if they were one.
test-memory-static-embed: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/memory_static_embed_tests.pas -o$(BUILDDIR)/memory_static_embed_tests
	@$(BUILDDIR)/memory_static_embed_tests
...
# Session import/export: ChatGPT conversations.json (mapping-tree walk),
# Claude Code .jsonl, native round-trip, Markdown export.
test-session-port: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/session_port_tests.pas -o$(BUILDDIR)/session_port_tests
	@PASCLAW_HOME=$(BUILDDIR)/session-port-test-home $(BUILDDIR)/session_port_tests
...
# GuardFall regression: ShellAllowed blocks the Adversa AI bash-obfuscation
# bypass classes (quote removal, $IFS, command substitution, decode-and-run
# pipelines, rm-equivalent flags) without refusing ordinary dev commands.
test-guardfall: | $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/lib
	$(FPC) $(FPCFLAGS) src/tests/guardfall_tests.pas -o$(BUILDDIR)/guardfall_tests
	@$(BUILDDIR)/guardfall_tests
...
test: smoke test-doctor test-provider-retry test-hashline test-toolview test-anthropic-server-tools test-openai-server-tools test-tool-choice test-responses-tool-choice test-println-helper test-utf8-codepage-tag test-gemini-schema-strip test-markdown-render test-json-utf8-roundtrip lint-utf8-file-io test-stats-attribution test-sqlite-hint test-memory-search-roundtrip test-relay-servertools test-shell-cwd-report test-shell-denylist-bypass test-skills-provenance test-ssrf-guard test-read-file-encoding test-db-tools test-json-lenient test-rerank test-sentencepiece test-rerank-eval test-model-discovery test-cron-tool test-provider-catalog test-output-cache test-agents test-working-state test-compact test-prune test-ansi-width test-shell-filters test-learn test-export test-execute-code test-session-stats test-auto-router test-gateway-stats-buckets test-session-list-filter test-session-endpoints test-config-secret-merge test-skills-install test-self-improving-skills test-loop-shaping-defaults test-prompt-batching test-turn-clock test-max-iter-notice test-tool-cancel test-delphi-iface-types test-plan-build-mode test-config-profile test-tool-rpc test-session-search test-session-port test-subagent-bg test-subagent-default test-subagent-model-arg test-zip-pack test-http-proxy test-stream-reliability test-mcp-server test-mcp-hub-projection test-checkpoints test-condense-json test-goals-runner test-kb-index test-kb-pdf test-agents-md test-checkpoints-zpaq test-orient-preamble test-component-config test-autoroute-apply test-fallback-models test-memory-distill test-memory-facts test-memory-static-embed test-memory-autodistill test-checkpoints-redo test-build-roundtrip test-promptware test-orient test-condense-reversible test-heartbeat test-shell-backend test-env-inject test-run-timeout test-shell-output-decode test-fs-grep-tier1-4 test-fs-grep-tier5-6 test-fs-tool-naming test-workspace-paths test-guardfall test-skills-prompt test-mcp-result test-mcp-compact test-config-mcp-flags test-workflow test-progress-ledger test-otel test-logger-level-quiet test-gateway-token test-fs-secret-gate test-config-env-subst test-delphi-build test-desktop lint-version
