...
# Glyphs the CODE draws (a TPath it builds itself), rather than glyphs a style
# book defines for a button lookup. Emitted as SVG path strings into an include
# file so the geometry has exactly one source: a hand-written path literal in
# MasterDetail.pas would be a second owner of the same shape, and this project
# has paid for that mistake more than once.
GLYPH_INC = "studio/PasclawGlyphs.inc"
...
# A speech bubble: body plus a tail. Drawn at row scale in the sessions list,
# so it stays blunt -- corners are chamfers, not arcs.
_BUBBLE = [[(0.06, 0.10), (0.94, 0.10), (0.94, 0.68), (0.52, 0.68),
            (0.30, 0.95), (0.30, 0.68), (0.06, 0.68)]]

# name -> subpaths, emitted to GLYPH_INC as SVG path data for code to draw
CODE_GLYPHS = {
    "SESSION": _BUBBLE,
}
...
def svg_path(subpaths):
    """SVG path text -- TPathData.Data parses this directly."""
    out = []
    for sub in subpaths:
        out.append("M" + " L".join("%.4f,%.4f" % (x, y) for x, y in sub) + " Z")
    return " ".join(out)


def build_inc():
    lines = [
        "{ GENERATED by scripts/gen-studio-icons.py -- do not edit.",
        "",
        "  Path data for glyphs the code draws itself. Same geometry source as",
        "  the button styles in the .style books, so a shape cannot differ",
        "  between a button and a hand-built TPath. }",
    ]
    for name in sorted(CODE_GLYPHS):
        lines.append("  GLYPH_%s = '%s';" % (name, svg_path(CODE_GLYPHS[name])))
    return "\n".join(lines) + "\n"
...
def main(argv):
    ...
    inc = build_inc()
    try:
        have_inc = open(GLYPH_INC, encoding="utf-8", errors="replace").read()
    except OSError:
        have_inc = None
    if have_inc == inc:
        print("%s: up to date (%d glyphs)" % (GLYPH_INC, len(CODE_GLYPHS)))
    elif check:
        print("%s: OUT OF DATE -- run scripts/gen-studio-icons.py" % GLYPH_INC)
        bad += 1
    else:
        open(GLYPH_INC, "w", encoding="utf-8", newline="").write(inc)
        print("%s: wrote %d glyphs" % (GLYPH_INC, len(CODE_GLYPHS)))
